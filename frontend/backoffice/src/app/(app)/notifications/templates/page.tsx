...
function getAvailableVariables(eventType: string): Array<{ path: string; description: string }> {
  const variables: Record<string, Array<{ path: string; description: string }>> = {
    ...
    REPAIR_CREATED: [
      { path: 'payload.repairId', description: 'ID ремонта' },
      { path: 'payload.unitId', description: 'ID квартиры' },
      { path: 'payload.unitName', description: 'Название квартиры' },
      { path: 'payload.unitAddress', description: 'Адрес квартиры' },
      { path: 'payload.scheduledAt|date', description: 'Дата и время (форматированная)' },
      { path: 'payload.masterId', description: 'ID мастера' },
      { path: 'payload.masterName', description: 'Имя мастера' },
      { path: 'payload.notes', description: 'Примечания' },
      { path: 'payload.orgId', description: 'ID организации' },
    ],
    REPAIR_ASSIGNED: [
      { path: 'payload.repairId', description: 'ID ремонта' },
      { path: 'payload.unitId', description: 'ID квартиры' },
    ...
    REPAIR_INSPECTION_COMPLETED: [
      ...
      { path: 'payload.masterName', description: 'Имя мастера' },
      { path: 'payload.scheduledAt|date', description: 'Запланировано (форматированная дата)' },
      { path: 'payload.submittedAt|date', description: 'Осмотр завершен (форматированная дата)' },
      { path: 'payload.notes', description: 'Примечания' },
      { path: 'payload.checklistStats.total', description: 'Всего пунктов чеклиста' },
      { path: 'payload.checklistStats.completed', description: 'Выполнено пунктов' },
      { path: 'payload.checklistStats.incomplete', description: 'Не выполнено пунктов' },
      { path: 'payload.photoUrls', description: 'Массив URL фотографий' },
    ],
    REPAIR_STARTED: [
      { path: 'payload.repairId', description: 'ID ремонта' },
      { path: 'payload.unitName', description: 'Название квартиры' },
    ...
    REPAIR_CANCELLED: [
      ...
      { path: 'payload.masterId', description: 'ID мастера' },
      { path: 'payload.masterName', description: 'Имя мастера' },
      { path: 'payload.scheduledAt|date', description: 'Запланировано было (форматированная дата)' },
      { path: 'payload.reason', description: 'Причина отмены' },
      { path: 'payload.notes', description: 'Примечания' },
    ],
  };

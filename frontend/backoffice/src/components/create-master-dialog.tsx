...
import { CREATE_MASTER, GET_USERS } from '@/lib/graphql-queries'
...
export function CreateMasterDialog({
  ...
  const createMasterMutation = useMutation({
    mutationFn: (input: any) => graphqlClient.request(CREATE_MASTER, { input }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['masters'] })
      queryClient.refetchQueries({ queryKey: ['masters'] })
      handleClose()
    },
    onError: (error: any) => {
      alert(`Ошибка при создании мастера: ${error.message || 'Неизвестная ошибка'}`)
    }
  })
  ...
  const handleCreate = async () => {
    ...
    createMasterMutation.mutate(input)
  }
  ...
  return (
    <Dialog open={isOpen} onClose={handleClose} size="2xl">
      ...
      <DialogDescription>
        Добавьте нового мастера в систему. Мастер должен быть связан с существующим пользователем.
      </DialogDescription>
      <DialogBody className="space-y-6">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
          ...
          <Text className="text-sm text-blue-800 dark:text-blue-200">
            ...
            из профиля пользователя автоматически, и ему будет добавлена роль <strong>MASTER</strong>.
          </Text>
        </div>
          ...
          <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-700">
            ...
            <div className="mt-3 pt-3 border-t border-green-200 dark:border-green-700">
              <Text className="text-xs text-green-700 dark:text-green-300">
                После создания пользователю будет добавлена роль <strong>MASTER</strong>
              </Text>
            </div>
          </div>
        )}
        ...
        <div className="bg-gray-50 dark:bg-zinc-800 p-4 rounded-lg">
          ...
          <ul className="mt-2 space-y-1 text-sm text-gray-600 dark:text-gray-400">
            <li>• Создается запись Master с типом INTERNAL</li>
            <li>• Имя и email берутся из профиля User</li>
            <li>• Пользователю добавляется роль MASTER</li>

    ...
    try {
        ...
        if (allSubscriptions.length > 1) {
            logger.warn('⚠️ Found multiple NOTIFICATION subscriptions, deactivating duplicates', {
                ...
                ids: allSubscriptions.map((s) => s.id)
            });
        ...
        const existing = allSubscriptions.find((s) => s.isActive) || allSubscriptions[0];
        const allEventTypes = [
            // Booking events
            'BOOKING_CREATED',
            'BOOKING_CONFIRMED',
        ...
        if (!existing) {
            ...
            try {
                const subscription = await prisma.eventSubscription.create({
                    data: {
                        handlerType: 'NOTIFICATION', // Приводим к типу HandlerType enum
...
try {
    const verification = await prisma.eventSubscription.findFirst({
        where: {
            handlerType: 'NOTIFICATION',
            isActive: true
        }
    ...
    if (verification) {
        ...
        const allSubs = await prisma.eventSubscription.findMany({});

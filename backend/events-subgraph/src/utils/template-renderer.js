...
export class TemplateRenderer {
    ...
    static render(template, context) {
        ...
        return template.replace(regex, (match, expression) => {
            try {
                ...
                if (path.endsWith('priceAmount') && !filters.some((f) => f.startsWith('currency'))) {
                    filters.push('currency');
                }
                // Автоматически добавляем фильтр difficultyLabel для cleaningDifficulty, если он не указан
                if (path.endsWith('cleaningDifficulty') && !filters.some((f) => f.startsWith('difficultyLabel'))) {
     ...
     */
    static applyFilters(value, filters, context) {
        let result = value;
